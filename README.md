...
or a post-2025 health outcome. Its NYCCAS values are preliminary observed monitor concentrations.**
...
- Release `2026-09-18.1` is built from the raw source downloads and served by the app.
...
- **CROSSINGS** module: published MTA facility crossing summaries with source-register facility names.
- **CRZ** module: published monthly entry context by detection group, labeled as descriptive counts.
- **AIR** module: preliminary NYCCAS PM2.5 monitor measurements at daily and on-demand hourly
  resolution, with missing values visible, playback, source checksums, and non-causal labeling.
...
- **EQUITY / CONFIDENCE / HOTSPOTS** render an explicit "Not available for this claim" state.
  Their material (2023 DAC layers, historical asthma records, and hotspot rankings) is deliberately
...
- CI runs lint, tests, build, and the release gate; automated accessibility coverage exists for key
  rendered states. A full browser E2E suite is still missing.
- The prior production deployment remains release `2026-09-16.2`; release `2026-09-18.1` is a locally built candidate and has not been deployed. Rollback is manual.
...
- `data/catalog/sources.yaml` — the source register (14 sources)
...
```
data/releases/2026-09-18.1/
  manifest.json                 release index: assets, checksums, coverage, limitations
  traffic_observations.geojson  2,082,788 bytes, EPSG:4326
  facility_crossings.json       157,586 bytes
  nyccas_pm25_daily.csv         1,564,746 bytes, 15 monitors
  nyccas_pm25_hourly.csv       41,705,713 bytes, on-demand hourly playback
...
The repository also retains the newer `2026-09-20.1` and `2026-09-20.2` release artifacts from
`origin/main`, including the normalized CRZ entry summary, for traceability and follow-up release
work. The browser-facing pointer remains on `2026-09-18.1` so the AIR/PM2.5 module continues to load
its declared assets.
...
- `scripts/release_acceptance.py` — the deployment gate (49 checks in the current merged build)
...
byte** (verified with the SHA-256 values in `data/releases/2026-09-18.1/manifest.json`).
...
`WebSite` structured data describing the published release, its window, its distributions, and the
sources it is based on. A `<noscript>` block links the manifest and published assets directly, so the
...
```bash
npm run test                              # 173 tests
python3 scripts/release_acceptance.py     # 49 checks, blocks a bad deploy
