...
data/releases/2026-09-20.4/
  ...
  crz_entry_summary.json        monthly CRZ detection-group entries
  air_context.json              modelled historical air context
  health_context.json           historical health context
  equity_context.geojson        archived DAC geography
...
support for manifests that publish `air_measurements`; the browser-facing pointer is the newer
`2026-09-20.4` release from `origin/main`.
...
byte** (verified with the SHA-256 values in `data/releases/2026-09-20.4/manifest.json`).
...
```bash
npm run test                              # see current Vitest output
