...
```bash
npm run test                                  # 199 tests: accessibility, claim and size gates
...
npx tsc -b
npm run build
python3 scripts/release_acceptance.py          # 58 checks
...
| All blocking acceptance tests pass before a production deploy | Gate run before each deploy (58 checks); CI runs the gate, both suites, and the size ratchet on push | Met |
