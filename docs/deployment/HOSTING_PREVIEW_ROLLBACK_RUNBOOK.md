...
## Verified target facts (re-checked 2026-09-21)

| Fact | Value | How it was verified |
|---|---|---|
| Firebase account project count | 5 projects | `firebase projects:list --json` (CLI, authenticated as `pivot.hopes@gmail.com`) |
| Project display name / ID | `tollshallows` / `tollshallows` | `firebase projects:list --json` |
| Project number | `406249496898` | `firebase projects:list --json` |
| Default Hosting site | `tollshallows` — `https://tollshallows.web.app` | `firebase hosting:sites:list --project tollshallows --json` |
| Singular `tollshallow` | not accessible to this account — HTTP 403 | `firebase hosting:sites:list --project tollshallow` |
| Pre-release plural site state | HTTP 200, legacy manifest generated 2026-09-11 with no `release_id` | `curl https://tollshallows.web.app/data/manifest.json` |

The repository `.firebaserc` now names the owner-confirmed plural project; see "Configuration change" below.

**Do not treat a new candidate as deployed.** The plural site is serving, but every future release still needs the preview and smoke checks below.
...
This rejects a candidate that is missing a build, marked `synthetic: true`, unvalidated, missing required release metadata, missing an asset checksum, shipping raw/archive payloads, over the browser-asset budget, or carrying credentials — and it rejects a `.firebaserc` that does not name `tollshallows`.
...
```bash
firebase hosting:channel:deploy <release-id> --project tollshallows --expires 7d
```

Always pass `--project tollshallows` explicitly. Do not rely on the default from `.firebaserc`, and never pass the singular project. The command prints the preview URL; record it.
...
```bash
firebase deploy --only hosting --project tollshallows
```

Then re-run the smoke checks against `https://tollshallows.web.app/` and record the result. The candidate is only considered live after this step returns the expected release pointer and SPA shell.
...
```
$ npx firebase hosting:releases:list --project tollshallows
Error: hosting:releases:list is not a Firebase command
$ npx firebase hosting:channels:delete <channel-id> --project tollshallows
...
npx firebase hosting:channel:deploy <release-id> --project tollshallows --expires 7d
...
npx firebase hosting:channel:delete <channel-id> --project tollshallows --force
...
- Deploying to `tollshallow` (singular) or omitting `--project`.
...
`.firebaserc` was corrected from `tollshallow` to the owner-confirmed `tollshallows` project. The canonical, Open Graph, sitemap, robots, security-contact, structured-data, and acceptance-gate origins use the same plural site. Rationale and fresh CLI evidence are in the table above.
