...
**Owner:** Agent 3
**Status:** executed and rehearsed. Production serves release `2026-09-20.3`, deployed 2026-09-20 after a green gate. The rollback section below was rehearsed on preview channel `p10-rehearsal` on 2026-09-20 against the installed CLI, which is how the two non-existent commands it used to name were found. The steps below are the standing procedure.
...
| Current singular site state | HTTP 200, serving release `2026-09-20.3` | `curl https://tollshallow.web.app/data/manifest.json` |
...
2. **Asset budget is sized to the release, not surveyed.** The gate's 8 MiB `dist/data` ceiling is a working figure; the current release payload is 6.02 MiB across six assets. It rises when a release adds a context layer and should be re-derived rather than nudged.
