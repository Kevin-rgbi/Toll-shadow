...
and the declared CSVs, either in a processed-data workspace or the retained
data/reference-legacy copies. It says nothing about whether the policy labels,
hourly estimate, facility mapping, or coordinates are correct.
...
def source_candidates(processed_root: Path, source_path: str) -> list[Path]:
    """Candidate files for a baseline `source_path`, including retained legacy references."""
    relative = Path(source_path)
    candidates: list[Path] = []
    for root in source_roots(processed_root):
        candidates.append(root / relative)
        candidates.append(root / "data" / "reference-legacy" / relative.name)
    return candidates
...
def resolve_source(processed_root: Path, source_path: str) -> Path | None:
    ...
    for candidate in source_candidates(processed_root, source_path):
        if candidate.exists():
            return candidate
    return None
