...
def gate(repo_root: Path, dist: Path, budget_bytes: int, dist_overridden: bool = False) -> Gate:
    ...
    for entry in asset_entries:
        ...
        # source/method link. A register identifier is not a link, so an asset without a resolvable
        # URL is an unlinked metric and is refused here rather than rendered that way.
        ...
        if not isinstance(source_urls, list) or not source_urls:
            ...
            bad = [url for url in source_urls if not isinstance(url, str) or not url.startswith(("http://", "https://"))]
            result.check(
                f"asset publishes a source URL: {label}",
                not bad,
