...
import { loadMeasureSpecification } from '../src/measure-spec.mjs';
...
describe('buildRelease', () => {
  it('builds immutable browser-safe DOT and MTA assets with source-quality evidence', async () => {
    ...
    // Take the release id from the measure spec rather than pinning it here, so this test does not
    // need editing every time a release is cut.
    const specification = await loadMeasureSpecification(path.join(repositoryRoot, 'pipeline/methods/release-1.yaml'));
    const result = await buildRelease({
      releaseId: specification.release_id,
