 ...
 */
/**
 * `MapShell` is back on the ratchet because the merge keeps the release-layer split from main while
 * retaining the local AIR/PM2.5 release-point hover and selection path. `releaseData` validates both
 * the current context assets and the legacy monthly/CRZ shapes needed by retained releases. `RasterMap`
 * remains the WebGL-free fallback renderer, now with the merged accessibility and release-layer paths.
 * These are merge allowances, not new targets: lower them whenever a focused split lands.
 */
const RECORDED: Record<string, number> = {
  'src/App.tsx': 640,
  'src/components/Map/MapShell.tsx': 485,
  'src/components/Map/RasterMap.tsx': 680,
  'src/lib/releaseData.ts': 520,
}
